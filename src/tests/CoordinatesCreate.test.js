...
describe('CoordinatesCreate', () => {
    ...
    it('displays an error message for an invalid token', async () => {
        ...
        fireEvent.change(getByLabelText(/Coordinates Name/i), { target: { value: 'TestName' } });
        fireEvent.change(getByLabelText(/Latitude/i), { target: { value: '100' } });
        fireEvent.change(getByLabelText(/Longitude/i), { target: { value: '10000' } });
        ...
        await waitFor(() => {
          expect(getByText(/Invalid latitude or longitude values. Latitude should be in the range -90 to 90, and longitude should be in the range -180 to 180./i))
        });
      });
